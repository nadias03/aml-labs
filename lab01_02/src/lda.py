...
class LDA:
    ...
    def fit(self, X, y):
        ...
        Sigma_hat /= (n - 2)
        Sigma_hat += self.lambda_ * np.eye(p)   # np.eye - macierz jednostkowa (to mnozenie tu to zabezpieczenie sie przed macierza nieodwracalna)
    ...
    def _log_dens_k(self, X, k):
        ...
        _, Sigma_logdet = np.linalg.slogdet(Sigma)   # numpy liczenie logarytmu z wyznacznika macierzy jakies optymalne
        X_c = X - mu
    ...
    def predict_proba(self, Xtest):
        ...
        log_joint_prob_1 = log_dens_1 + log_prior_1   # w notatkach (*)
        log_joint_prob_0 = log_dens_0 + log_prior_0   # w notatkach (<3)

        delta = log_joint_prob_1 - log_joint_prob_0

        posterior_1 = 1 / (1 + np.exp(-delta))   # NIE OGARNIAM TEGO
