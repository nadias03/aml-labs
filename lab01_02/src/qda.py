...
class QDA:
    ...
    def fit(self, X, y):
        ...
        # covariance matrices
        Sigma_hat_0 = np.zeros((p, p))
        X_0c = X_0 - mu_0_hat
        Sigma_hat_0 += X_0c.T @ X_0c
        Sigma_hat_0 /= (n_0 - 1)
        Sigma_hat_0 += self.lambda_ * np.eye(p)

        Sigma_hat_1 = np.zeros((p, p))
        X_1c = X_1 - mu_1_hat
        Sigma_hat_1 += X_1c.T @ X_1c
        Sigma_hat_1 /= (n_1 - 1)
        Sigma_hat_1 += self.lambda_ * np.eye(p)
        ...
        self.Sigma_hat_ = np.array([Sigma_hat_0, Sigma_hat_1])
        self.pi_ = np.array([pi_0, pi_1])

    def _log_dens_k(self, X, k):
        mu = self.mu_hat_[k]
        Sigma = self.Sigma_hat_[k]
        p = self.p_
